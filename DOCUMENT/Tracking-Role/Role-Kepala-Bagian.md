...
Scope canonical role ini: **bawahan langsung dari relasi efektif**. `employees.export` default **OFF**; grant/revoke matrix tetap efektif dan seluruh output mengikuti scope/filter/privacy. `users.switch_role` configurable; bila ON target: **Pegawai**; same/higher/invalid/chained ditolak. Permission tidak mengganti identitas/employee ownership dengan user lain. PATEN self/assignment yang sah tidak membutuhkan checkbox; manual dan cancellation memakai permission + scope/domain, bukan allowlist role permanen.
